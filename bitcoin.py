...
def computeDelta(wt, X, Xi):
    ...
    for i in xrange(0,len(Xi)):
        y = Xi.iloc[i][-1] #how to use iloc is inspired from sklearn documentation
        x = Xi.iloc[i][:-1]
        #inspired formula from www.stackoverflow.com
        sim = float(sum((X[:-1]-np.mean(X[:-1]))*(x-np.mean(x))))/float(len(X[:-1])*np.std(X[:-1])*np.std(x))
        num =  num+y * math.exp(sim * wt)#formula from paper
        den =  den+math.exp(sim * wt)
...
model = smf.ols('deltaP ~ deltaP90+deltaP180+deltaP360',data = trainData).fit()
